<%@ Page Title="" Language="C#" MasterPageFile="~/Operations.master" AutoEventWireup="true"
    CodeFile="StoresReturn.aspx.cs" Inherits="StoresReturn" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <script type="text/javascript">
        ...
        $(document).click(function () {
            ...
            price = $row.find('#txt_perunitrs').val(),
        ...
        function removerow(thisid) {
            ...
            for (var i = 0; i < DummyTable1.length; i++) {
                results += '<tr><td scope="row" class="1" style="text-align:center;" id="txtsno">' + j + '</td>';
                results += '<td ><span id="spn_Productname">' + DataTable[i].productname + '</span><input id="txtProductname" readonly   class="productcls" style="width:90px;display:none" value="' + DummyTable1[i].productname + '"/></td>';
                results += '<td style="display:none;" class="2">' + DummyTable1[i].productname + '</td>';
                results += '<td ><input id="txt_perunitrs" type="text" class="price" onkeypress="return isFloat(event)" style="width:90px;" value="' + DummyTable1[i].PerUnitRs + '"/></td>';
        ...
        function save_Stores_Return() {
            var doe = document.getElementById('txt_inwarddate').value;
            ...
            $('#tabledetails> tbody > tr').each(function () {
                ...
                if (hdnproductsno == "" || hdnproductsno == "0") {
                    ...
                    fillitems.push({ 'txtsno': txtsno, 'productname': productname, 'PerUnitRs': PerUnitRs, 'Quantity': Quantity, 'hdnproductsno': hdnproductsno, 'sisno': sisno
                    });
                }
            ...
            var Data = { 'op': 'save_Stores_Return', 'doe': doe, 'name': name, 'remarks': remarks, 'sno': sno, 'btnval': btnval, 'fillitems': fillitems };
            var s = function (msg) {
                if (msg) {
                    alert(msg);
                    get_Stores_details();
        ...
        function fillstores(msg) {
            var results = '<div  style=""><table class="table table-bordered table-hover dataTable no-footer" role="grid" aria-describedby="example2_info">';
            results += '<thead><tr class="trbgclrcls"><th scope="col">Name</th><th scope="col">Remarks</th><th scope="col"></th></tr></thead></tbody>';
        ...
        function update(thisid) {
            ...
            var doe2 = $(thisid).parent().parent().children('.3').html();
            var sno = $(thisid).parent().parent().children('.4').html();

            var doe1 = doe2.split('-');
            var doe = doe1[2] + '-' + doe1[1] + '-' + doe1[0];

            document.getElementById('txt_inwarddate').value = doe;
            document.getElementById('txt_Name').value = name;
            document.getElementById('txt_remarks').value = remaks;
            ...
            for (var i = 0; i < stores_subdetails.length; i++) {
                if (sno == stores_subdetails[i].sstore_refno) {
                    ...
                    results += '<td data-title="From"><span id="spn_Productname">' + stores_subdetails[i].productname + '</span><input id="txtProductname" readonly class="productcls"  name="productname" value="' + stores_subdetails[i].productname + '" style="width:100%; font-size:12px;padding: 0px 5px;height:30px;display:none;"></input></td>';
                    results += '<td data-title="From"><input class="price" id="txt_perunitrs" name="PerUnitRs" value="' + stores_subdetails[i].PerUnitRs + '" style="width:100%; font-size:12px;padding: 0px 5px;height:30px;"></input></td>';
                    results += '<td data-title="From"><input class="quantity" id="txt_quantity" onkeypress="myFunction()" name="Quantity" value="' + stores_subdetails[i].quantity + '" style="width:100%; font-size:12px;padding: 0px 5px;height:30px;"></input></td>';
    ...
    <section class="content">
        <div class="box box-info">
            <div class="box-header with-border">
                <h3 class="box-title">
                    <i style="padding-right: 5px;" class="fa fa-cog"></i>Stores Return Details
                </h3>
            </div>
            <div class="box-body">
                <div id="showlogs" align="center">
                    <%--<input id="add_Inward" type="button" name="submit" value='Stores Return' class="btn btn-primary" />--%>
                    <div class="input-group" style="padding-left:87%">
                        <div class="input-group-addon">
                            <span class="glyphicon glyphicon-plus-sign"></span> <span id="add_Inward">Stores Return</span>
                        </div>
                    </div>
                </div>
                <div id="div_inwardtable" style="padding-top:3px;">
                </div>
                <div id='vehmaster_fillform' style="display: none;">
                    <table align="center">
                        ...
                        <tr>
                            ...
                            <td style="height: 40px;">
                                <input id="txt_Name" class="form-control" type="text" placeholder="Enter Name" onKeyPress="return ValidateAlpha(event);" />
                            </td>
                        </tr>
                        <tr>
                            <td>
                                <label>
                                    Remarks</label><span style="color: red;">*</span>
                            </td>
                            <td colspan="4">
                    ...
                    <table align="center">
                        <tr>
                            <td>
                                <%--<input type="button" class="btn btn-primary" id="btn_RaisePO" value="Save" onclick="save_Stores_Return();" />
                                ...
                                <table>
                                   <tr>
                                    ...
                                    <div class="input-group">
                                        <div class="input-group-addon">
                                        <span class="glyphicon glyphicon-ok" id="btn_RaisePO1" onclick="save_Stores_Return()"></span> <span id="btn_RaisePO" onclick="save_Stores_Return()">Save</span>
                                  </div>
                                  </div>
                                    </td>
