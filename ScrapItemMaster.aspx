<%@ Page Title="" Language="C#" MasterPageFile="~/Operations.master" AutoEventWireup="true" CodeFile="ScrapItemMaster.aspx.cs" Inherits="ScrapItemMaster" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    <link href="autocomplete/jquery-ui.css" rel="stylesheet" type="text/css" />
    <script type='text/javascript'>
        $(function () {
            get_ScrapItem_details();
        ...
        function saveScrapItemDetails() {
            ...
            var data = { 'op': 'saveScrapItemDetails', 'ScrapItemName': productname, 'MainCode': productcode, 'SKU': sku, 'Description': description, 'categoryid': ddlcategory, 'subcategoryid': ddlsubcategary, 'subcode': subcode, 'uim': uim, 'ProductId': Productid, 'price': Price, 'btnVal': btnval };
            var s = function (msg) {
                if (msg) {
                    if (msg.length > 0) {
                        alert(msg);
        ...
        var productdetails = [];
        function get_ScrapItem_details() {
            var data = { 'op': 'get_ScrapItem_details' };
            var s = function (msg) {
                if (msg) {
                    if (msg.length > 0) {
                        BindGrid(msg);
        ...
        function BindGrid(msg) {
            ...
            for (var i = 0; i < msg.length; i++) {
                ...
                results += '<td scope="row" class="1">' + msg[i].ItemName + '</td>';
        ...
        function filllikeproduct(msg) {
            ...
            for (var i = 0; i < ScrapItemdetails1.length; i++) {
                results += '<tr style="background-color:' + COLOR[l] + '"><td><input id="btn_poplate" type="button"  onclick="getme(this)" name="submit" class="btn btn-primary" value="Edit" /></td>';
                results += '<th scope="row" class="1" style="text-align:center;">' + ScrapItemdetails1[i].ItemName + '</th>';
                results += '<td   style="display : none;"  class="4">' + ScrapItemdetails1[i].category + '</td>';
                results += '<td   class="6">' + ScrapItemdetails1[i].Itemcode + '</td>';
                results += '<td   class="5">' + ScrapItemdetails1[i].sku + '</td>';
                  results += '<td   class="9">' + ScrapItemdetails1[i].subcategoryid + '</td>';
                results += '<td   class="16">' + ScrapItemdetails1[i].price + '</td>';
                results += '<td   class="2">' + ScrapItemdetails1[i].description + '</td>';
        ...
        function test2() {
            ...
            if (sku == "") {
                ...
                for (var i = 0; i < ScrapItemdetails.length; i++) {
                    results += '<tr style="background-color:' + COLOR[l] + '"><td><input id="btn_poplate" type="button"  onclick="getme(this)" name="submit" class="btn btn-primary" value="Edit" /></td>';
                    results += '<th scope="row" class="1" style="text-align:center;">' + ScrapItemdetails[i].ItemName + '</th>';
                    results += '<td   style="display : none;"  class="4">' + ScrapItemdetails[i].category + '</td>';
                    results += '<td   class="6">' + ScrapItemdetails[i].Itemcode + '</td>';
                    results += '<td   class="5">' + ScrapItemdetails[i].sku + '</td>';
                     results += '<td   class="9">' + ScrapItemdetails[i].subcategoryid + '</td>';
                     results += '<td   class="16">' + ScrapItemdetails[i].subcode + '</td>';
                     results += '<td   class="16">' + ScrapItemdetails[i].price + '</td>';
                    results += '<td   class="2">' + ScrapItemdetails[i].description + '</td>';
                    results += '<td   style="display : none;" class="11">' + ScrapItemdetails[i].uom + '</td>';
                    results += '<td   style="display : none;" class="12">' + ScrapItemdetails[i].itemid + '</td><tr>';
                ...
                for (var i = 0; i < ScrapItemdetails.length; i++) {
                    if (sku == ScrapItemdetails[i].sku) {
                        ...
                        results += '<td   class="16">' + ScrapItemdetails[i].price + '</td>';
                        results += '<td   class="2">' + ScrapItemdetails[i].description + '</td>';
                        results += '<td   class="2">' + ScrapItemdetails[i].subcode + '</td>';
                        results += '<td   style="display : none;" class="11">' + ScrapItemdetails[i].uom + '</td>';
                        results += '<td   style="display : none;" class="12">' + ScrapItemdetails[i].itemid + '</td><tr>';
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li><a href="#">Scrap Item Master</a></li>
        </ol>
    </section>
    <section class="content">
        <div class="box box-info">
            <div class="box-header with-border">
                <h3 class="box-title">
                    <i style="padding-right: 5px;" class="fa fa-cog"></i>Scrap Item Details
                </h3>
            </div>
            <div class="box-body">
                <div id="showlogs" align="center">
                    <%--<table>--%>
                       <%-- <tr>
                            ...
                            <%--<tr>
                            <td>
                                <input id="btn_addProduct" type="button" name="submit" value='Add Scrap Item' class="btn btn-primary"
                                    onclick="showdesign();" />
                            </td>
                            <td style="width: 10px">
                            </td>
                    ...
                    <div id="show_department" class="input-group" style="padding-left:87%;">
                      <div class="input-group-addon">
                        <span class="glyphicon glyphicon-plus-sign" onclick="showdesign()"></span> <span onclick="showdesign()">Add Scrap Item</span>
                      </div>
                </div>
                </div>
                
                <div id="div_ProductData" style="padding-top:2px;">
                </div>
                    ...
                    <table align="center" style="width: 60%;">
                        ...
                        <tr>
                            <td style="height: 40px;">
                               <label>Scrap Item Name</label><span style="color: red;">*</span>
                               <input id="txtProductName" type="text" maxlength="45" class="form-control" name="vendorcode"
                                    placeholder=" Enter Scrap Item Name" /><label id="lbl_code_error_msg" class="errormessage">*
                                        Please Enter Scrap Item Name</label>
                            </td>
                            <td style="width: 5px;">
                            </td>
                            <td style="height: 40px;">
                        ...
                        <tr>
                            <td colspan="2" align="center" style="height: 40px;padding-left: 18%;">
                                <%--<input id="btn_save" type="button" class="btn btn-primary" name="submit" value='save'
                                    onclick="saveScrapItemDetails()" />
                                ...
                                <table>
                                   <tr>
                                    ...
                                    <div class="input-group">
                                        <div class="input-group-addon">
                                        <span class="glyphicon glyphicon-ok" id="btn_save1" onclick="saveScrapItemDetails()"></span> <span id="btn_save" onclick="saveScrapItemDetails()">save</span>
                                  </div>
                                  </div>
                                    </td>
